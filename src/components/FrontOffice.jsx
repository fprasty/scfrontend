...
setCanSubmit(false);
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/api/focreate`, {
        "lokasi":"Front Office",
        "status":"Menunggu",
  ...
  const cekStatus = async (e) => {
    try {
    const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/focek`);
