...
const TOmShin = () => {
  ...
  const createUser = async (e) => {
    ...
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/api/toscreate`, {
        "lokasi":"Tahu Om Shin",
        "status":"Menunggu",
        "antre": generateRandomNumber(),
        "created_at": moment().format("dddd, MMMM Do YYYY, h:mm:ss a"),
      });
      navigate("/tahuomshin/page");
    } catch (error) {
      if (error.response) {
  ...
  const cekStatus = async (e) => {
    try {
    const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/toscek`);
